...
The repo's default GPU pool is `ADA_24` (24 GB Ada — RTX 4090). The `MinerU2.5-Pro-2605-1.2B` VLM on the MinerU 3.4.x runtime fits in 24 GB with KV-cache headroom. In the measurements below, the 4090 was both faster per page and cheaper per page than the lower-hourly A5000/3090 (`AMPERE_24`). This page covers the cases where a different trade-off makes sense.
...
- **The [20 MB response cap](/blog/2026-05-20-runpod-20mb-response-cap-r2-bridge/)** — a few hundred pages of output already blows past it.
- **No resumability** — one job has no checkpoint; a failure means starting over.

The fix is **client-side page-range batching**, not a 48 GB card. The worker slices server-side: pass `start_page`/`end_page` ([API reference](/reference/api/)) and each job parses one contiguous range out of the same uploaded file — you never pre-split the PDF. Keep `MINERU_MAX_CONCURRENCY=1` and scale throughput with `workers_max` (more GPU workers, not a bigger GPU). See [Concurrency](/guides/concurrency/) for how `workers_max` and `MINERU_MAX_CONCURRENCY` interact.

<Callout type="info" title="The worker does not auto-batch">
It parses one page range per job; splitting a large document into batches is something *you* do on the client. The full walkthrough — clause-aligned boundaries, R2 transport for huge output, and the throughput math — is in [Clause-aligned batching for large PDFs](/blog/2026-06-03-clause-aligned-batching-large-pdf-mineru/), where a **5,039-page** standard parsed on a 24 GB A5000 for ~$1.15.
</Callout>

## Picking a backend

MinerU 3.4.x exposes five backends through this template. Select one per job; the same worker can handle different backends on consecutive jobs.
...
Pipeline trades raw speed for memory predictability: page-by-page streaming with no vLLM KV-cache accumulation, **4 GB minimum** per MinerU's hardware compatibility table. Per-page time is **~3–5 s across GPUs** for pipeline. The VLM backend is much more GPU-sensitive and content-sensitive: MinerU upstream reports **~0.5 s/page (2.12 fps) on an A100**; we measured **~1 s/page (uniform reports) to ~6 s/page (dense forms) on RTX 4090**, and **~1–10 s/page on A5000 24 GB** depending on content density. MinerU 3.4's pipeline backend provides preferred script-family models for non-Latin input. The Pro VLM handled Cyrillic in our test even though `lang` is ignored on the VLM path, but that test does not establish broader script coverage.
...
Preferred MinerU 3.4 `lang` codes are script or model families rather than ISO codes: `east_slavic`, `cyrillic`, `arabic`, `devanagari`, `korean`, `ta`, `te`, `ka`, `th`, `el`, `ch`, and `ch_server`. Older aliases such as `en`, `latin`, `japan`, and `chinese_cht` are accepted for compatibility but normalize to `ch`. See [Input formats](/guides/input-formats/#the-lang-parameter-pipeline-backend-only) for the current table.

### Hybrid: measure it on your documents

`hybrid-auto-engine` routes work between the pipeline and VLM paths. MinerU's upstream compatibility table gives hybrid the same 8 GB minimum as the VLM backend, but this project does not yet publish representative peak-VRAM measurements for hybrid workloads. Do not move to 48 GB based on document length or the backend name alone. Start on 24 GB, measure peak VRAM on representative documents, and move up only for an observed OOM or deliberate in-worker concurrency.

**Empirical note (Pro-2604 + Cyrillic)**: we A/B-tested `vlm-auto-engine` against `hybrid-auto-engine` on Cyrillic fiscal PDFs (forms with embedded tables and codes) and got **near-identical output** — same Cyrillic transcription, same cell-collision pattern on dense form headers. Hybrid did not improve that test set. We have not repeated the A/B test on Pro-2605, so treat the result as historical evidence for one model and corpus, not a current coverage guarantee. Reach for hybrid when layout disambiguation is the suspected bottleneck, then verify it on your documents.
