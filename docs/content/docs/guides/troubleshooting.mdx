...
| `server_url must point at a publicly routable host` | You set `MINERU_ENFORCE_TARGET_POLICY=1` and your model server is on a private address | Set `MINERU_ALLOW_LOCAL_FETCH=1` as well — the two compose, giving you the policy plus an exemption for your own server. Unset `MINERU_ENFORCE_TARGET_POLICY` only if every caller of the endpoint is trusted, since the field comes from the job payload |
