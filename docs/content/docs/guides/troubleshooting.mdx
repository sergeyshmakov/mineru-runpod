...
| `server_url must point at a publicly routable host` | You set `MINERU_ENFORCE_TARGET_POLICY=1` and your model server is on a private address | Add the host to `MINERU_ALLOWED_SERVER_HOSTS`, which exempts that one name and nothing else. Not `MINERU_ALLOW_LOCAL_FETCH` — that lifts the policy globally, re-admitting any private `server_url` a caller sends and disabling the check on `file_url` too |
