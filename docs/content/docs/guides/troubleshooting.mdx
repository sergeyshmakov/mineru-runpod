...
- **Endpoint-side**: raise the endpoint's execution timeout — 3600 s for full books. Set it on the endpoint itself (RunPod console, or a `saveEndpoint` call): `deploy.py --execution-timeout` only reports the value it would want, because the SDK's `create_endpoint` exposes no parameter for it.
