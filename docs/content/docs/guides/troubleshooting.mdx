...
| `server_url must be an http(s) URL` | Same shape check on the `*-http-client` backend field — often a `host:port` with no scheme | Write it out: `http://10.1.2.3:8000/v1` |
