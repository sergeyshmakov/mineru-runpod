...
- `server_url` must resolve to a publicly routable address too, under the same `MINERU_ALLOW_LOCAL_FETCH=1` switch. **This changed:** it used to take the shape check only, on the reasoning that where you run your own vLLM server is your call. But `server_url` arrives in the job payload rather than the endpoint environment, so any caller could point the worker at a loopback or link-local address — a cloud metadata endpoint included — and have it issue requests there from inside your network. Set the variable if your model server is private.
...
7. **URL fields.** `file_url` and `server_url` must be `http(s)` with a host, and both must resolve to a publicly routable address unless `MINERU_ALLOW_LOCAL_FETCH=1`. See [URL inputs](#url-inputs).
