...
- `server_url` gets the shape check by default, and the routable-address policy only when you set `MINERU_ENFORCE_TARGET_POLICY=1`. **Worth understanding, because the default is not the safe one.** `server_url` arrives in the job payload rather than the endpoint environment, so any caller of your endpoint can point the worker at a loopback or link-local address — a cloud metadata endpoint included — and have it issue requests there from inside your network. The default is compatibility: a private model server is the ordinary way to run one, and enforcing the policy by default would break every such deployment on a patch upgrade. **If your endpoint is reachable by callers you do not control, set `MINERU_ENFORCE_TARGET_POLICY=1`.** If your model server is itself on a private address, name its host in `MINERU_ALLOWED_SERVER_HOSTS` (comma-separated, exact host match). Do **not** use `MINERU_ALLOW_LOCAL_FETCH` for this — that flag lifts the address policy for every field and every target, so it would re-admit arbitrary private `server_url` values from any caller and switch off the same protection on `file_url`.
...
7. **URL fields.** `file_url` and `server_url` must be `http(s)` with a host. `file_url` must also resolve to a publicly routable address unless `MINERU_ALLOW_LOCAL_FETCH=1`; `server_url` gets that policy only under `MINERU_ENFORCE_TARGET_POLICY=1`. See [URL inputs](#url-inputs).
