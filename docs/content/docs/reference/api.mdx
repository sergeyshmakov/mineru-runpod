...
7. **URL fields.** `file_url` and `server_url` must be `http(s)` with a host. `file_url` must also resolve to a publicly routable address unless `MINERU_ALLOW_LOCAL_FETCH=1`. Under `MINERU_ENFORCE_TARGET_POLICY=1`, `server_url`'s host must appear in `MINERU_ALLOWED_SERVER_HOSTS`; `MINERU_ALLOW_LOCAL_FETCH` does not affect it. See [URL inputs](#url-inputs).
