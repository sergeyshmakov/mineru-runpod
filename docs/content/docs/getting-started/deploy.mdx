...
| `execution_timeout` | `900 s` | Per-job cap; covers a several-hundred-page parse. `deploy.py` reports this value rather than setting it — the SDK's `create_endpoint` has no parameter for it, so set it on the endpoint after creation. |
