---
title: Clients
description: Call mineru-runpod with the Python client, the RunPod SDK, or direct HTTP, with current request and response examples.
---
...
Before you start, you need a [deployed endpoint](/getting-started/deploy/), its
endpoint id, and a RunPod API key. For the Python examples, keep the key out of
source control:
...
<Callout type="info" title="Coming from the MinerU cloud API?">
If you already call the official MinerU API (`mineru.net/api/v4/...`), the `MineruApiClient` facade lets you evaluate and migrate to a self-hosted endpoint with a near-identical code path. See [Migrate from the MinerU API](/getting-started/migrate-from-mineru-api/).
</Callout>
...
For asynchronous work, `endpoint.run(...)` returns a job object. Streaming is
on that job, not on the endpoint:
...
See the [RunPod Python SDK docs](https://docs.runpod.io/sdks/python/endpoints) for the full surface.
...
The raw `/runsync` HTTP response is a RunPod envelope, so the handler result is
under `output`. The Python SDK unwraps that envelope and returns the handler
result directly.
...
Full job-input shape is in [API reference](/reference/api/).
...
| You need streaming progress for long parses | **B — SDK `Job.stream()`** |
...
Add `formats: ["markdown"]` to the input if you only want the markdown back — the other three keys are then absent from the entry. See [Output modes](/guides/output-modes/) for the full transport × formats matrix.

Failure responses set `ok: false`, include a top-level `error` key, and have NO `results` field; RunPod marks the job FAILED in the dashboard. See [API reference](/reference/api/) for the full schema.
