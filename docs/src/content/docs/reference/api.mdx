...
| Field | Type | Notes |
|---|---|---|
| `basename` | string | Echo of the input `basename`, useful when batches arrive |
...
| `tarball_b64` | string | Present when `transport: "tarball_b64"`. Base64-encoded `.tar.gz` of the output directory |
...
| `tarball_url`, `tarball_url_expires_in`, `bucket_key`, `bucket_bytes` | various | Present when `transport: "s3"`. See [Output modes → s3](/mineru-runpod/guides/output-modes/) |

:::note[Reserved for batch input]
A future batch-input phase will add per-entry `ok` and `error` fields so a single-job multi-file submission can report partial success. Today's single-file responses do not include them.
:::
