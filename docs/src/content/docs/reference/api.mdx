...
| `probe` | bool | `false` | Diagnostic mode. `{"probe": true}` skips parsing and every field above, then returns the worker's filesystem layout plus the running MinerU version instead of a parse result — for checking baked model paths and volume mounts. On by default; set `MINERU_ENABLE_PROBE=0` on the endpoint to turn it off, on endpoints whose callers should not see filesystem paths or model-cache details. `MINERU_DISABLE_PROBE=1` is still honoured. |
