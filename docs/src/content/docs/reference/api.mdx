...
| `end_page` | int | `-1` | 0-based, inclusive. Any negative value or omission means "to end of document" |
...
3. **Basename safety.** Every character in `basename` must be alphanumeric, `-`, or `_`. Unicode letters and digits are accepted.
4. **Inline file size.** The worker rejects decoded `file_b64` data over 20 MB. RunPod's whole-request limits are reached earlier because base64 expands the raw file; use the practical thresholds above.
