...
The `runpod-mineru` repo wraps all of this into one Docker image: MinerU 3.2.x + the `MinerU2.5-Pro-2605-1.2B` VLM, the JobScaler-bypass composition for warmup, structured logging, and the rest. Open source ([GitHub](https://github.com/sergeyshmakov/mineru-runpod)), MIT-licensed, deploys from the RunPod Hub in two clicks.

If you want the deeper breakdown of which phases of a cold start cost what, the [troubleshooting guide](/guides/troubleshooting/#flashboot-mechanism-confirmed) has the per-phase timing table from the same test runs. The [scaling guide](/guides/scaling/#flashboot) covers when to pair FlashBoot with `workers_min ≥ 1` for fully predictable latency.
