...
:::note[API note]
The job-input field `return` was renamed to `transport` (with an orthogonal `formats` filter for inline outputs), and success responses now wrap each parsed file in a `results: [...]` list — see the current [API reference](/reference/api/). Snippets below predate that change and use the older `return` field; they're left intact as the post's original record.
:::
...
- **20 MB response cap.** RunPod's `/runsync` gateway drops responses over ~20 MB. Multi-page parses with embedded images hit this around 50–80 pages. Worker logs `done`; client gets `NoneType`. Fix: `return: "s3"` + Cloudflare R2, walked through in [the R2 bridge post](/blog/2026-05-20-runpod-20mb-response-cap-r2-bridge/).
