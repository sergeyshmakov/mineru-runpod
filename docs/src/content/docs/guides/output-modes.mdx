---
title: Output modes
description: Pick between tarball_b64, inline, and S3 presigned URLs with the transport field, and use formats to filter which artifacts the inline payload carries.
---

The worker hands back parsed output through two orthogonal knobs:

- **`transport`** — how the output is shipped (`tarball_b64` / `inline` / `s3`).
- **`formats`** — for inline transport, which subset of `markdown` / `content_list` / `middle` / `images` rides along in the response.

Pick `transport` based on **how big the output is** and **what you want to do with it**. Use `formats` to skip artifacts you don't need.
...
| `"inline"` | entry has `markdown` + `content_list` + `middle` + `images` (each base64, filterable via `formats`) | Same ~20 MB cap | You want structured fields back, easy to consume in JSON-only callers; small docs |
| `"s3"` | entry has `tarball_url`: presigned URL to the same `.tar.gz` on an S3-compatible bucket | No practical limit (bucket-side) | Large docs (long books, image-heavy reports); response stays tiny |
...
Unpack with `MineruClient.save_tarball(result, "./out")` or any base64 + gzip + tar tooling. **Default** because it round-trips cleanly through any JSON layer and the output dir on disk matches what MinerU produces locally.
...
For inline transport, `formats` whitelists which of the four artifacts ride back in the entry. Omitted keys are absent from the response — not present-as-empty. Useful when only the markdown matters and the images would otherwise add megabytes of base64 to the payload.
...
Response entry contains only `markdown` (plus the usual `basename` / `source` / `pages_requested`). `content_list`, `middle`, and `images` are not present.
...
- Order doesn't matter. Duplicates collapse silently.
- Empty list is rejected (`formats: []` → validation error). Omit the field to get all four.
- `formats` is a **no-op** for `tarball_b64` and `s3` transports. Those always ship the full tarball — filtering the contents of an archive would just confuse callers who later download it and find a missing file. If you want a partial set with those transports, extract the tarball and ignore the files you don't need.
...
If you need a longer lifetime, fork the worker and bump `S3_PRESIGN_TTL_SECONDS` in [`worker/package.py`](https://github.com/sergeyshmakov/mineru-runpod/blob/main/worker/package.py). I haven't exposed this as a per-job knob because the default is rarely too short — a job that needs >1 h to fetch its output usually has a different bug.
