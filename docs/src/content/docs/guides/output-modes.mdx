...
The worker uses one transport choice plus an inline-only filter:
...
If you need a longer lifetime, fork the worker and change `S3_PRESIGN_TTL_SECONDS` in [`worker/package.py`](https://github.com/sergeyshmakov/mineru-runpod/blob/main/worker/package.py). It is not a per-job setting.
