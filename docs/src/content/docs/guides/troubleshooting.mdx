...
When something doesn't work, start with the `debug` block. Success and failure responses include the requested backend, detected input format, GPU, a best-effort cached model path, and per-phase timings.
...
| `input_format` | Auto-detected from bytes. If you supplied a PDF URL and see `unknown`, the URL likely returned an error page (HTML), not the file body |
