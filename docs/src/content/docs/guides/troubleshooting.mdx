...
| `probe is disabled on this endpoint` | The endpoint sets `MINERU_ENABLE_PROBE=0` (or the older `MINERU_DISABLE_PROBE=1`) | Read the layout from a worker log line, or set `MINERU_ENABLE_PROBE=1` on an endpoint you own |
