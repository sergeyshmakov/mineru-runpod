...
The five MinerU backends (see [Picking a backend](/runpod-mineru/guides/choosing-gpu/#picking-a-backend)) handle the input pool differently:

- **`vlm-auto-engine`** (default): MinerU's end-to-end VLM. Best for **PDF**. MinerU's README attaches the [109-language claim](https://github.com/opendatalab/MinerU#supported-languages) specifically to the pipeline backend's OCR layer; the VLM's [model card](https://huggingface.co/opendatalab/MinerU2.5-Pro-2604-1.2B) carries only `English` and `Chinese` HuggingFace language tags. Empirically the VLM transliterates non-Latin scripts (Cyrillic, Arabic, Devanagari) to Latin glyphs rather than producing the real characters. For non-Latin content, prefer `pipeline` with a script-family `lang` code.
...
- **`hybrid-auto-engine`**: routes per page. Best when you don't know in advance what mix of layouts you have.
...
The VLM backends (`vlm-*`, `hybrid-*` with VLM routing) **ignore the `lang` field**. Passing `lang: "east_slavic"` with `backend: "vlm-auto-engine"` does nothing — you'll get Latin-transliterated nonsense on Cyrillic input. This is one of the most common silent failures; see the [Troubleshooting guide](/runpod-mineru/guides/troubleshooting/#vlm-backend-output-is-latinized-on-non-english-input).
...
Required combination. Without `backend: "pipeline"`, the VLM transliterates Cyrillic to nearest-Latin glyphs.
...
For multi-hundred-MB books or large image archives, prefer `file_url` (signed S3 URL is cheapest) or pre-stage on a network volume — see [Network volumes](/runpod-mineru/guides/network-volumes/).
