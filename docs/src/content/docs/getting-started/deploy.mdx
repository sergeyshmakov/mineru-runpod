...
An endpoint id like `abcdef123456` that accepts the documented [job input contract](/reference/api/) via RunPod's standard `/run` and `/runsync` endpoints. From there, see [Clients](/getting-started/clients/) for how to actually call it from your code.
